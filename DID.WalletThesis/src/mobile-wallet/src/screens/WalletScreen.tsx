...
function LockScreen({ onUnlock, colors }: LockScreenProps) {
  const [mode, setMode] = useState<"checking" | "biometric" | "pin">(
    "checking",
  );
  ...
  if (mode === "biometric") {
    return (
      ...
      >
        ...
        {error && (
          <Text style={[lockStyles.errorText, { color: "#dc2626" }]}>
            {error}
          </Text>
        )}
        <TouchableOpacity
  ...
  return (
    ...
    >
      <View
        style={[lockStyles.iconWrap, { backgroundColor: colors.primaryLight }]}
      >
      ...
      <View style={lockStyles.numpad}>
        {DIGITS.map((d, i) =>
          d === "" ? (
            <View key={i} style={lockStyles.numpadCell} />
          ) : (
            <TouchableOpacity
              key={i}
              style={[
                lockStyles.numpadCell,
                lockStyles.numpadBtn,
                { backgroundColor: colors.surface, borderColor: colors.border },
              ]}
              onPress={
                d === "⌫" ? pressBack : () => pin.length < 6 && pressDigit(d)
              }
              activeOpacity={0.7}
              disabled={locked}
            >
              <Text style={[lockStyles.numpadBtnText, { color: colors.text }]}>
                {d}
              </Text>
            </TouchableOpacity>
          ),
...
export default function WalletScreen() {
  ...
  const surfaceColor = themeAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [LIGHT_COLORS.surface, DARK_COLORS.surface],
  });
  const borderColor = themeAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [LIGHT_COLORS.border, DARK_COLORS.border],
  });
    ...
    <Animated.View style={[styles.safe, { backgroundColor: bgColor }]}>
      ...
      >
        <View style={styles.topBarLeft}>
          <View
            style={[styles.topBarIconWrap, { backgroundColor: colors.primary }]}
          >
          ...
          >
            <View style={[styles.idCard, { backgroundColor: colors.primary }]}>
              ...
              <View style={styles.idCardHeader}>
                <Text style={styles.idCardTypeLabel}>
              ...
              <Text style={styles.idCardAddress} numberOfLines={1}>
                ...
                {identity.keys.map((key, i) => (
                  ...
                  >
                    ...
                    <View style={{ flex: 1, marginLeft: 12 }}>
                      <Text
                        style={[styles.keyTypeName, { color: colors.text }]}
                      >
                        {key.type}
                      </Text>
                      <Text
                        style={[styles.keyHex, { color: colors.textMuted }]}
                        numberOfLines={1}
                      >
                        {key.publicKeyHex.slice(0, 20)}…
                        {key.publicKeyHex.slice(-8)}
                      </Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => copy(key.publicKeyHex, key.kid)}
                    >
                      <Text
                        style={[
                          styles.copySmall,
...
const styles = StyleSheet.create({
  ...
  topBar: {
    ...
    paddingTop:
      Platform.OS === "android" ? (StatusBar.currentHeight ?? 0) + 12 : 68,
    paddingBottom: 18,
    borderBottomWidth: 1,
  },
